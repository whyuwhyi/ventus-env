...
int main() {
  ...
  std::vector<uint32_t> output = runBinaryKernel<uint32_t>("kernel.cl", "vmul_bf16x2", a, b);
  ...
  for (size_t i = 0; i < N; ++i) {
    ...
    uint32_t ref = packBf16x2(aLo * bLo, aHi * bHi);
    pass += output[i] == ref;
  }
  std::printf("vmul_bf16x2: %d/%zu pass\n", pass, N);
