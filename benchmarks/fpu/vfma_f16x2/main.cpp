...
int main() {
  ...
  for (size_t i = 0; i < N; ++i) {
    ...
    c[i] = packF16x2(dist(rng), dist(rng));
  }

  std::vector<uint32_t> output = runTernaryKernel<uint32_t>("kernel.cl", "vfma_f16x2", a, b, c);
  ...
  for (size_t i = 0; i < N; ++i) {
    ...
    float cLo = halfBitsToFloat(uint16_t(c[i] & 0xffffu));
    float cHi = halfBitsToFloat(uint16_t(c[i] >> 16));
    uint32_t ref = packF16x2(aLo * bLo + cLo, aHi * bHi + cHi);
    pass += output[i] == ref;
  }
  std::printf("vfma_f16x2: %d/%zu pass\n", pass, N);
