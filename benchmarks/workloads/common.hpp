...
inline double eventDurationNs(cl_event event) {
  cl_ulong start = 0;
  cl_ulong end = 0;
...
inline uint32_t packF16x2Bits(uint16_t lo, uint16_t hi) {
  return uint32_t(lo) | (uint32_t(hi) << 16);
}

inline uint32_t packF16x2(float lo, float hi) {
  return packF16x2Bits(floatToHalfBits(lo), floatToHalfBits(hi));
}
