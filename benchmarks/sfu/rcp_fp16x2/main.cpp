...
int main() {
  ...
  for (size_t i = 0; i < N; ++i) {
    inLo[i] = dist(rng);
    inHi[i] = dist(rng);
    input[i] = packF16x2(inLo[i], inHi[i]);
  }

  auto ctx = createContext();
  cl_program program = buildProgram(ctx, "kernel.cl");
  cl_int err = CL_SUCCESS;
  cl_kernel kernel = clCreateKernel(program, "rcp_fp16x2", &err);
  CHECK_CL(err);
  ...
  for (size_t i = 0; i < N; ++i) {
    float outLo = halfBitsToFloat(uint16_t(output[i] & 0xffffu));
    float outHi = halfBitsToFloat(uint16_t(output[i] >> 16));
    float refLo = 1.0f / inLo[i];
    float refHi = 1.0f / inHi[i];
  ...
  std::printf("rcp_fp16x2: %d/%zu lane-pass, maxRelErr=%e\n", pass, N * 2,
              maxRelErr);
